...
function AssessmentResults({ results, formData, onReset }) {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      downloadPdfReport(pdfBytes, `iso-9001-readiness-${safeName}-${dateStr}.pdf`);
    } catch (err) {
    ...
    <div className="max-w-4xl mx-auto">
      ...
      {gaps.length === 0 && (
        <div className="rounded-xl border border-green-600/40 bg-green-900/20 px-6 py-6 text-center mb-6">
          ...
          <p className="text-sm text-gray-400">
            Your organization appears to meet all assessed ISO 9001 requirements. Consider a formal certification audit to validate.
          </p>
        </div>
      )}
      ...
      <div className="mt-8 rounded-xl border border-purple-600/40 bg-purple-900/20 px-6 py-6 text-center">
        <h3 className="text-lg font-semibold text-gray-100 mb-2">Need help improving your QMS?</h3>
        <p className="text-sm text-gray-400 mb-4">
          Our ISO 9001 experts can help you build a quality management system and prepare for certification.
        </p>
        <Link
          to="/quote"
